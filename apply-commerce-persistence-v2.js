...
async function applyPersistenceSchema() {
    try {
        console.log('Connecting to database...');
        const pool = await sql.connect(config);
        console.log('Connected.');

        const schemaPath = path.join(__dirname, 'database', 'commerce_persistence_v2.sql');
        ...
        console.log('Commerce Persistence Schema applied successfully.');
        process.exit(0);
    } catch (err) {
