...
export function ChatInterface({
  ...
  // Current chat ID for persistence
  ...
  const sendMessage = useCallback(
    ...
    [sessionId, uploadImageToS3, locale, t]
  );

  // Update chat title on first message
  useEffect(() => {
    if (messages.length === 1 && messages[0].role === "user" && appModeCtx) {
      appModeCtx.updateChatTitle(messages[0].text.slice(0, 50));
    }
  }, [messages, appModeCtx]);
