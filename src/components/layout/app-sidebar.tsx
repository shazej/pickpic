...
function SidebarContent({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const filteredChats = chats.filter((c) => c.mode === mode);
  ...
  const groupedChats = {
    today: filteredChats.filter((c) => c.createdAt >= today.getTime()),
    ...
    older: filteredChats.filter((c) => c.createdAt < weekAgo.getTime()),
  };
      ...
      <div className="px-3 pb-3">
        <div className="flex rounded-lg bg-gray-800/80 p-1">
          ...
          >
            ...
            {t("sidebar.buyMode" as TranslationKey)}
          </button>
          <button
          ...
          >
            ...
            {t("sidebar.sellMode" as TranslationKey)}
          </button>
        </div>
      </div>
