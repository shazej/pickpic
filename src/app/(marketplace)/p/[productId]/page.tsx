...
export default async function ProductPage({ params }: { params: Promise<{ productId: string }> }) {
    try {
        ...
        const imagesResult = await query(
            `SELECT image_url, alt_text FROM marketplace.ProductImages WHERE product_id = @productId ORDER BY is_primary DESC, created_at ASC`,
            [{ name: 'productId', value: productId, type: sql.UniqueIdentifier }]
        );
        ...
        const images = imagesResult.recordset.map((img: any, i: number) => ({
            id: i.toString(),
            url: img.image_url,
            alt: img.alt_text || dbProduct.title
        }));
                        ...
                        <Tabs defaultValue="details">
                            ...
                            <TabsContent value="reviews" className="mt-4">
                                <div className="grid gap-8">
                                    <ReviewList />
                                    {/* removed due to serialization issues */}
                                </div>
                            </TabsContent>
                        </Tabs>
                    </div>
